...
## [2026-06-25] 接入京麦平台已收款资金明细同步功能
*   **功能更新**：
    *   **一键书签捕获默认值预设与弹窗免输入优化**：为了免去王某配置时的额外输入流程，在前台书签脚本生成逻辑（`App.tsx`）中针对京麦平台预设了默认的账户 ID `'110266763006'` 和商户名称 `'京东商户结算中心'`。如果捕获阶段未通过 Cookie/URL 成功匹配到 ID，则直接采用默认值作为 fallback，消除了弹窗询问的问题，实现点击直接免密捕获上报。
    *   **重构京东资金明细同步字段**：升级了 `jingmai_finance` 的 Schema 结构，根据王某的要求，定义了“主键ID”（主键）、“收款到账日期”、“账单日期”、“收入”、“支出”、“实际结算”等 6 个核心财务字段。
    *   **接入京麦已收款资金明细真实接口**：在后端 `dy_helper.js` 中实现了对接京麦核心接口 `/capital/completeData.do` 进行真实的账户明细拉取。在 Cookie 缺失或失效时，进行非静默 `CredentialsExpired` 异常抛出。
    *   **高仿真收支流水 Mock 数据生成**：为京麦已收款资金明细模块设计并生成了 5 条高度仿真的 Mock 财务交易流水（包括到账时间、账单日期、收入、支出和实际结算，时区归档展现），全面满足王某本地无感化调试需求。
    *   **物理隔离与非静默异常抛出**：重构了 `table_records.js` 的真实拉取异常捕获，实现了对于真实 Cookie 校验失效/过期的非静默降级机制。当检测到真实凭据但网络拉取发生异常或遇到登录网关拦截时，系统将坚决拒绝 Mock 兜底降级，直接抛出 `CredentialsExpired` 凭证失效异常至飞书多维表格端，保障企业生产数据的真实合规。

## [2026-06-25] 接入千牛工作台收支明细同步功能
*   **功能更新**：
    *   **一键书签捕获默认值预设与弹窗免输入优化**：为了免去王某配置时的额外输入流程，在前台书签脚本生成逻辑（`App.tsx`）中针对千牛平台预设了默认的店铺 ID `'499066699'` 和店铺名称 `'高原安旗舰店:财务'`。如果捕获阶段未通过 Cookie/URL 成功匹配到 ID，则直接采用默认值作为 fallback，**彻底消除了在千牛页面点击书签脚本时弹出 prompt 询问的问题，实现点击直接免密捕获上报**。
    *   **千牛数据同步崩溃热修复**：修复了 `table_records.js` 的 catch 块中生成 19 条高原安旗舰店财务流水时的错误，移除了多余的 raw `item` push 操作，彻底解决了因数据格式不一引发的排序崩溃 `Cannot read properties of undefined (reading 'col_record_time')`。
    *   **重构收支明细同步字段**：升级了 `qianniu_fund_detail` 的 Schema 结构，根据王某的要求，定义了“入账时间”、“支付流水号”（主键）、“淘宝订单号”、“入账类型”、“收入金额”、“支出金额”、“业务描述”、“备注”等 8 个核心财务字段。
    *   **接入千牛收支明细真实 Mtop 接口**：在后端 `dy_helper.js` 中实现了对接淘宝核心接口 `mtop.taobao.finance.fund.bill.query` 进行真实的账户明细拉取，能剥离 jsonp 回调、自适应解析 JSON 并清洗出入账时间、金额借贷归集等。在 Cookie 缺失或失效时，进行非静默 `CredentialsExpired` 异常抛出。
    *   **高仿真收支流水 Mock 数据生成**：为千牛收支明细模块设计并生成了 20 条高度仿真的 Mock 财务交易流水（包括订单交易收入、退款支出、服务费扣减、直通车推广推广费、保证金增充等典型账目，且收支金额分别列归档展现），全面满足王某本地无感化调试需求。
    *   **物理隔离与非静默异常抛出**：重构了 `table_records.js` 的真实拉取异常捕获，实现了对于真实 Cookie 校验失效/过期的非静默降级机制。当检测到真实凭据但网络拉取发生异常或遇到 CSP 登录网关拦截时，系统将坚决拒绝 Mock 兜底降级，直接抛出 `CredentialsExpired` 凭证失效异常至飞书多维表格端，保障企业生产数据的真实合规。
    *   **智能防空与自动装配**：在拉取接口前集成了针对 `qianniu` 平台的 Cookie SQLite 自动读取机制，当同步连接缺少凭证时会自动检索刚刚在千牛登录并上报的真实凭据进行装配。

## [2026-06-25] 接入聚水潭 ERP 登录测试与库存同步功能
*   **功能更新**：
    *   **重构库存同步字段**：升级了 `jushuitan_inventory` 的 Schema 结构，根据王某的要求，定义了“序号”、“图片”、“款式编码”、“商品编码”（主键）、“商品名称”、“颜色及规格”、“商品标签”、“实际库存数”、“订单占有数”、“可用数量”、“库存可售天数”等 11 个业务字段，并打通了前后端数据流。
    *   **接入 GetPageListV2 库存接口**：在后端 `dy_helper.js` 中实现了对聚水潭 ERP 官方库存同步接口（`https://apiweb.erp321.com/webapi/ItemApi/ItemSku/GetPageListV2`）的对接。
    *   **网络传输自适应与 Fallback 机制**：默认通过 POST 形式向聚水潭 ERP 接口发起 Cookie 凭证代理请求。并集成了自适应重试机制，当遇到 POST 请求返回 405 (Method Not Allowed) 等异常时，会自动降级使用带查询参数的 GET 方法发起兜底请求，保障数据提取的高可用性。
    *   **响应解析与数据解包优化**：补充了对 `resJson.data.items` 及外层直接返回 `items` 结构的自动解析逻辑，确保能稳健地清洗出聚水潭商品的 SKU 数据。
    *   **高仿真库存 Mock 模拟数据**：为聚水潭库存模块定制了 25 条带 Unsplash 真实商品图片、商品编码、可用库存自动计算规则的高仿真模拟数据，大幅提升了王某在本地调试时的页面视觉回显体验。
    *   **聚水潭登录跳转与一键书签捕获**：
        *   在前端 `App.tsx` 中将聚水潭 ERP 的官方登录跳转地址对齐为主站 `https://www.erp321.com/`。
        *   优化了书签提取脚本中提取 `shopId` 的正则匹配规则，新增对 `owner_co_id` / `authorize_co_id` / `co_id` 的自动感知捕获，使得用户登录聚水潭系统后台后，只需点击书签即可一键上报 Cookie 凭证，极大提升了登录连接测试的体验。

## [2026-06-24] 支持小红书与多子系统动态网页登录、GET式书签上报规避CSP限制
*   **功能更新**：
    *   **小红书蒲公英真实笔记合作接口对接**：在后端 `dy_helper.js` 与 `table_records.js` 中完整接入并实现了小红书蒲公英笔记列表的真实拉取路由（`https://pgy.xiaohongshu.com/api/solar/content/note/list`），并在拉取阶段自动分流跳过抖音罗盘预检，完成了笔记 ID、标题、发布时间、阅读数及点赞数高保真属性数据清洗与飞书多维表格列的动态映射。
    *   **小红书蒲公英多维表格字段重构与 396 条分页 Mock 生成**：根据王某的要求，重构小红书推广-蒲公英达人合作笔记模块显示的 10 个字段，对齐了前端 `MODULE_FIELDS` 与后端 `table_meta` 架构；并在 Mock 模式下，支持基于 pageToken 的分页逻辑，在最近一个月的时区内高仿真生成并输出正好 396 条笔记记录。
    *   **小红书等多板块动态登录页面导航**：在前端 `App.tsx` 中引入了 `getDynamicLoginUrl`，支持根据用户选择的具体同步模块动态跳转至正确的系统登录页（如小红书蒲公英、小红书千帆、阿里妈妈淘宝联盟等），替代了之前固定的单一登录地址，极大提升了登录配置体验。
    *   **账号绑定与平台归类异常热修复**：
        *   后端 `database.js` 注入了基于 `module` 的智能平台归类兜底器（`detectPlatformByModule`），确保任何绑定请求都能精准归属对应的平台（如小红书、巨量千川等），防止平台丢失导致的归类错误。
        *   前端 `App.tsx` 重构了保存账号时的 fallback 逻辑，坚决隔离跨平台账号乱用，并补充了 `fetch` 请求的 `response.ok` 状态阻断与后端 500 报错的气泡提示。
        *   在 `initDb` 中执行了数据库迁移，自动修复了历史遗留的 platform 字段为 `null` 的账号数据，使其重新正常分类显现。
    *   **飞书多表同步时创建两个多维表格的容错优化**：
        *   重构了 `/api/table_meta` 与 `/api/records` 的 JSON 转换逻辑，自适应兼容字符串与对象的混用格式。
        *   增强了多维表格对于同步 `config` 直接解构的场景支持，避免了因为解析失败回退默认 `order_report` 导致重复建表的问题。
    *   **GET方式凭证上报规避CSP限制**：针对小红书等平台具有严格的内容安全策略（CSP）从而阻碍跨域发送 POST 请求的问题，将书签助手重构为通过 `window.open` 启动新页面进行 GET 上报。
    *   **后端GET收集端点与玻璃拟态回显**：在后端 `index.js` 中新增了 `GET /api/v1/connector/sources/login-capture-get` 路由以接收书签上报，捕获成功后自动将凭据存盘入库，并返回一个精美的毛玻璃（Glassmorphism）成功回显页面，并在 2 秒后自动执行 `window.close()`，完成操作闭环。
